...
export default Events = ({ navigation, route }) => {
  ...
  return (
    ...
    >
      <SearchBar
        show={showSearch}
        label="Busca por nombre del evento"
        value={search}
        setter={setSearch}
        action={searchEvents}
      />
              ...
              ListEmptyComponent={() =>
                foundEvents === undefined ? null : (
                  <InformationMessage
                    ...
                    description="No hay ningún evento registrado que cumpla con los parámetros de tu búsqueda"
                  />
                )
              }
              refreshing={loading}
              onRefresh={() => searchEvents()}
              renderItem={({ item }) => (
                <Item
                  key={item.event_identifier}
                  place={item.place}
                  belonging_area={item.belonging_area}
                  belonging_place={item.belonging_place}
                />
              )}
            />
          </Flex>
